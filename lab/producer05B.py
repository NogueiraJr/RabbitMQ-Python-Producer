...
def response_callback(ch, method, properties, body):
    if properties.correlation_id == correlation_id:
        ...
        # Extrair o ID da resposta (ajuste conforme o formato real da resposta)
        response_data = json.loads(response_message['text'])
        user_id = response_data.get('id')
        
        if user_id:
            # Nova mensagem a ser enviada para outro endpoint usando o ID
            new_message = {
                "method": "POST",
                "endpoint": "http://localhost:3333/another-endpoint",
                "headers": {
                    "Content-Type": "application/json"
                },
                "payload": {
            ...
            channel.basic_publish(exchange='',
                                  routing_key='user_queue',  # Pode ser uma fila diferente se necessário
